...
description: >-
  Trigger when: the task needs `budget-standard-checker` guidance as a focused standalone step or as part of the `gov-funding-plan` harness. Do not trigger when: the user wants the full `gov-funding-plan` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
