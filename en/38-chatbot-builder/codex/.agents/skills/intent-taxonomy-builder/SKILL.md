...
description: >-
  Trigger when: the task needs `intent-taxonomy-builder` guidance as a focused standalone step or as part of the `chatbot-builder` harness. Do not trigger when: the user wants the full `chatbot-builder` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
