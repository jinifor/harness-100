...
description: >-
  Trigger when: the task needs `license-compatibility-matrix` guidance as a focused standalone step or as part of the `open-source-launcher` harness. Do not trigger when: the user wants the full `open-source-launcher` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
