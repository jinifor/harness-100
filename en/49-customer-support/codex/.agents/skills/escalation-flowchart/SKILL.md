...
description: >-
  Trigger when: the task needs `escalation-flowchart` guidance as a focused standalone step or as part of the `customer-support` harness. Do not trigger when: the user wants the full `customer-support` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
