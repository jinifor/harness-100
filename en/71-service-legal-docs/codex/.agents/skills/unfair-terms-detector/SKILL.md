...
description: >-
  Trigger when: the task needs `unfair-terms-detector` guidance as a focused standalone step or as part of the `service-legal-docs` harness. Do not trigger when: the user wants the full `service-legal-docs` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
