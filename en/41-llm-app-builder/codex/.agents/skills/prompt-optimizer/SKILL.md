...
description: >-
  Trigger when: the task needs `prompt-optimizer` guidance as a focused standalone step or as part of the `llm-app-builder` harness. Do not trigger when: the user wants the full `llm-app-builder` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
