...
description: >-
  Trigger when: the task needs `wedding-budget-optimizer` guidance as a focused standalone step or as part of the `wedding-planner` harness. Do not trigger when: the user wants the full `wedding-planner` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
