...
description: >-
  Trigger when: the task needs `prior-art-search-strategy` guidance as a focused standalone step or as part of the `patent-drafter` harness. Do not trigger when: the user wants the full `patent-drafter` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
