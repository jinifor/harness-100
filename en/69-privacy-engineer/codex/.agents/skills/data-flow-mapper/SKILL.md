...
description: >-
  Trigger when: the task needs `data-flow-mapper` guidance as a focused standalone step or as part of the `privacy-engineer` harness. Do not trigger when: the user wants the full `privacy-engineer` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
