...
description: >-
  Trigger when: the task needs `learning-path-design` guidance as a focused standalone step or as part of the `onboarding-system` harness. Do not trigger when: the user wants the full `onboarding-system` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
