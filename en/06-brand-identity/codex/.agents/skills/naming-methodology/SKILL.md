...
description: >-
  Trigger when: the task needs `naming-methodology` guidance as a focused standalone step or as part of the `brand-identity` harness. Do not trigger when: the user wants the full `brand-identity` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
