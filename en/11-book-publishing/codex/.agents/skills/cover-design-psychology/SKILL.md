...
description: >-
  Trigger when: the task needs `cover-design-psychology` guidance as a focused standalone step or as part of the `book-publishing` harness. Do not trigger when: the user wants the full `book-publishing` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
