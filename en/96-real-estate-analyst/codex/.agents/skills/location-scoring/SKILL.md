...
description: >-
  Trigger when: the task needs `location-scoring` guidance as a focused standalone step or as part of the `real-estate-analyst` harness. Do not trigger when: the user wants the full `real-estate-analyst` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
