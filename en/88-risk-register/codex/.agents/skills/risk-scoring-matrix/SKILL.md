...
description: >-
  Trigger when: the task needs `risk-scoring-matrix` guidance as a focused standalone step or as part of the `risk-register` harness. Do not trigger when: the user wants the full `risk-register` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
