...
name: ats-optimizer
description: >-
  Trigger when: the task needs `ats-optimizer` guidance as a focused standalone step or as part of the `personal-branding` harness. Do not trigger when: the user wants the full `personal-branding` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
