...
description: >-
  Trigger when: the task needs `compound-interest-simulator` guidance as a focused standalone step or as part of the `personal-finance` harness. Do not trigger when: the user wants the full `personal-finance` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
