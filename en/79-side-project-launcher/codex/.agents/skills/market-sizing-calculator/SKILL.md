...
description: >-
  Trigger when: the task needs `market-sizing-calculator` guidance as a focused standalone step or as part of the `side-project-launcher` harness. Do not trigger when: the user wants the full `side-project-launcher` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
