...
description: >-
  Trigger when: the task needs `content-lifecycle-manager` guidance as a focused standalone step or as part of the `knowledge-base-builder` harness. Do not trigger when: the user wants the full `knowledge-base-builder` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
