...
description: >-
  Trigger when: the task needs `deduction-optimizer-engine` guidance as a focused standalone step or as part of the `tax-calculator` harness. Do not trigger when: the user wants the full `tax-calculator` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
