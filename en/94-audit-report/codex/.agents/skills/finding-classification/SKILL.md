...
description: >-
  Trigger when: the task needs `finding-classification` guidance as a focused standalone step or as part of the `audit-report` harness. Do not trigger when: the user wants the full `audit-report` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
