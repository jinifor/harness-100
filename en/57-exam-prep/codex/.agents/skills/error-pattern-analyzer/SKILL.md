...
description: >-
  Trigger when: the task needs `error-pattern-analyzer` guidance as a focused standalone step or as part of the `exam-prep` harness. Do not trigger when: the user wants the full `exam-prep` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
