...
description: >-
  Trigger when: the task needs `spatial-layout-guide` guidance as a focused standalone step or as part of the `space-concept-board` harness. Do not trigger when: the user wants the full `space-concept-board` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
