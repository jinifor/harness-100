...
description: >-
  Trigger when: the task needs `mobile-ux-patterns` guidance as a focused standalone step or as part of the `mobile-app-builder` harness. Do not trigger when: the user wants the full `mobile-app-builder` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
