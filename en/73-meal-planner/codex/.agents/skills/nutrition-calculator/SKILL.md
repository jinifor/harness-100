...
description: >-
  Trigger when: the task needs `nutrition-calculator` guidance as a focused standalone step or as part of the `meal-planner` harness. Do not trigger when: the user wants the full `meal-planner` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
