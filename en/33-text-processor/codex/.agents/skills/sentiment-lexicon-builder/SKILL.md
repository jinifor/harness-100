...
description: >-
  Trigger when: the task needs `sentiment-lexicon-builder` guidance as a focused standalone step or as part of the `text-processor` harness. Do not trigger when: the user wants the full `text-processor` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
