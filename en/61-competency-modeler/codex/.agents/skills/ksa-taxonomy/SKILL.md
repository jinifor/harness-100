...
name: ksa-taxonomy
description: >-
  Trigger when: the task needs `ksa-taxonomy` guidance as a focused standalone step or as part of the `competency-modeler` harness. Do not trigger when: the user wants the full `competency-modeler` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
