...
description: >-
  Trigger when: the task needs `audit-checklist-engine` guidance as a focused standalone step or as part of the `compliance-checker` harness. Do not trigger when: the user wants the full `compliance-checker` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
