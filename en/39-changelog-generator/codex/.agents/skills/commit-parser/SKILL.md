...
name: commit-parser
description: >-
  Trigger when: the task needs `commit-parser` guidance as a focused standalone step or as part of the `changelog-generator` harness. Do not trigger when: the user wants the full `changelog-generator` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
