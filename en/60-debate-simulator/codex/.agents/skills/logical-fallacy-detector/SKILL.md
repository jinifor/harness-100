...
description: >-
  Trigger when: the task needs `logical-fallacy-detector` guidance as a focused standalone step or as part of the `debate-simulator` harness. Do not trigger when: the user wants the full `debate-simulator` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
