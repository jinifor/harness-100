...
description: >-
  Trigger when: the task needs `permit-requirements-db` guidance as a focused standalone step or as part of the `regulatory-filing` harness. Do not trigger when: the user wants the full `regulatory-filing` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
