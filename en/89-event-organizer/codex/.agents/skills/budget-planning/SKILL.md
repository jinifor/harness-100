...
description: >-
  Trigger when: the task needs `budget-planning` guidance as a focused standalone step or as part of the `event-organizer` harness. Do not trigger when: the user wants the full `event-organizer` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
...
### cost structure (tablelevel allocation)
