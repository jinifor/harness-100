...
description: >-
  Trigger when: the task needs `viral-copywriting` guidance as a focused standalone step or as part of the `social-media-manager` harness. Do not trigger when: the user wants the full `social-media-manager` pipeline or an unrelated task outside this skill's domain. Expected input: brief, draft, issue, or source material. Expected output: domain-specific guidance, patterns, checks, or revisions.
---
